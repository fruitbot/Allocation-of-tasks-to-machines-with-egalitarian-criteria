...
# -*- coding: utf-8 -*-
"""
Created on Thu May 24 11:33:41 2018
...
def greedy_clust_best_eps_wo_lpt_typed(LM,LT,MCoeff,eps_precision=0.1):
    ...
    end_eps = float(2.0)
    start_eps=float(0.0)
    min_makespan = data_structure.cost_final_LM_gen( greedy_cluster_wo_lpt_typed(LM,LT,MCoeff,end_eps),MCoeff )
    ...
    while((math.fabs(end_eps - start_eps) > eps_precision)):
        ...
        A=data_structure.cost_final_LM_gen(greedy_cluster_wo_lpt_typed(LM,LT,MCoeff, (float)(math.fabs(end_eps+start_eps))/2.0),MCoeff)
    ...
    if data_structure.cost_final_LM_gen(greedy_cluster_wo_lpt_typed(LM,LT,MCoeff, start_eps),MCoeff) > data_structure.cost_final_LM_gen(greedy_cluster_wo_lpt_typed(LM,LT,MCoeff, end_eps),MCoeff):
...
def greedy_cluster_V2_wo_lpt_typed(LM, LT, MCoeff,esp):
    """ input : List of machines, List of tasks, Array of coefficients, Double for the coefficient*LB wanted
        output : new List of mahines
        builds clusters of tasks depending on their types. Then allocates those tasks with the LB criteria
    """
        ...
        if num_machine<len(LM_greedy):
            ...
            for task in cluster_copy:
                ...
                # case bad_esp 
                    ...
                    (LM_greedy[0]['LTM']).append(task)
                    cluster.remove(task)
            #The cluster is done, we change the machine
            num_machine += 1
        # case bad_esp
        else:
            cluster_copy = copy.deepcopy(cluster)
...
def greedy_clust_best_eps_V2_wo_lpt_typed(LM,LT,MCoeff,eps_precision=0.1):
    """input : list of machines * list of tasks * matrix types'coefficient * precision of eps i.e. 1.1 ? or 1.01 ? etc (by default : 0.1)
        output : the best eps that respect the following condition - all machine's cost must be < eps*LB
    ...
    end_eps = float(2.0)
    start_eps=float(0.0)
    min_makespan = data_structure.cost_final_LM_gen( greedy_cluster_V2_wo_lpt_typed(LM,LT,MCoeff,end_eps),MCoeff )
    
    # until eps' precision is not the one wanted
    while((math.fabs(end_eps - start_eps) > eps_precision)):
        # storing the (list of machines)'s cost when eps is the midpoint of start_eps and end_eps
        A=data_structure.cost_final_LM_gen(greedy_cluster_V2_wo_lpt_typed(LM,LT,MCoeff, (float)(math.fabs(end_eps+start_eps))/2.0),MCoeff)
    ...
    # finally we calculate which one between start_eps and end_eps holds the best eps
    # certainly the precision is the wanted one, but nothing says if the (list of machines)'s cost is lower with start_eps or end_eps
    if data_structure.cost_final_LM_gen(greedy_cluster_V2_wo_lpt_typed(LM,LT,MCoeff, start_eps),MCoeff) > data_structure.cost_final_LM_gen(greedy_cluster_V2_wo_lpt_typed(LM,LT,MCoeff, end_eps),MCoeff):
        #print("wo ",end_eps,min_makespan)
        return end_eps
    #print("wo ",start_eps,min_makespan)
    return start_eps


def greedy_clust_final_V2_wo_lpt_typed(LM,LT,MCoeff, k = 0):
    ...
    eps_precision=0.01
    eps = greedy_clust_best_eps_V2_wo_lpt_typed(LM,LT,MCoeff,eps_precision)
    return greedy_cluster_V2_wo_lpt_typed(LM,LT,MCoeff,eps)
    
def greedy_clust_final_wo_lpt_typed(LM,LT,MCoeff, k = 0):
    """input : list of machines * list of tasks * matrix types'coefficient * precision of eps i.e. 1.1 ? or 1.01 ? etc (by default : 0.0001)
        output : a new list of machines
    ...
    eps_precision=0.01
    eps = greedy_clust_best_eps_wo_lpt_typed(LM,LT,MCoeff,eps_precision)
    return greedy_cluster_wo_lpt_typed(LM,LT,MCoeff,eps)
