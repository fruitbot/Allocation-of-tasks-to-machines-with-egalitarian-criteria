...
def  order_greedy(All_types, MCoeff):

    ordered_types = []
    # get first 2 types, as to avoid that the 2 most incompatible types are together
    avoid_max = np.unravel_index(MCoeff.argmax(), MCoeff.shape)
    avoid_min = np.unravel_index(MCoeff.argmin(), MCoeff.shape)
    ...
    # find best of avoid_max
    minimum = float(min(MCoeff[avoid_max[0]]))
    x = avoid_max[0]
    y = MCoeff[avoid_max[0]].argmin()

    if float(min(MCoeff[avoid_max[1]])) < minimum:
        x = avoid_max[1]
        minimum = min(MCoeff[avoid_max[1]])
        y = MCoeff[avoid_max[1]].argmin()

    # find best of avoid_min
    maximum = float(max(MCoeff[avoid_min[0]]))
    x = avoid_min[0]
    y = MCoeff[avoid_min[0]].argmin()

    if float(max(MCoeff[avoid_min[1]])) > maximum:
        x = avoid_min[1]
        minimum = min(MCoeff[avoid_min[1]])
        y = MCoeff[avoid_min[1]].argmin()
